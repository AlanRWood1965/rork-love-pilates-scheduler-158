...
import { getFavouriteKey } from '@/providers/FavouritesProvider';
import ClassCard from '@/components/ClassCard';

const classTypeColorMap: Record<ClassType, string> = {
  Mat: Colors.mat,
...
export default function FavouriteClassesScreen() {
  const params = useLocalSearchParams<{ favouriteKey: string; title: string }>();
  const router = useRouter();
  const insets = useSafeAreaInsets();

  const favouriteKey = params.favouriteKey ?? '';
  const title = params.title ?? 'Favourite';
  ...
  const upcomingForFavourite = useMemo(() => {
    const todayStr = getTodayStr();
    return schedule
      .filter((c) => getFavouriteKey(c) === favouriteKey)
      .filter((c) => c.date >= todayStr)
      .sort((a, b) => {
        if (a.date !== b.date) return a.date.localeCompare(b.date);
        return a.time.localeCompare(b.time);
      });
  }, [schedule, favouriteKey]);

  const rows = useMemo<ListRow[]>(() => {
    const grouped: ListRow[] = [];
    let lastDate = '';
    for (const c of upcomingForFavourite) {
      if (c.date !== lastDate) {
    ...
    return grouped;
  }, [upcomingForFavourite]);

  const accentColor = useMemo(() => {
    const first = upcomingForFavourite[0];
    if (first) return classTypeColorMap[first.classType];
  ...
  const totalCount = upcomingForFavourite.filter((c) => !c.cancelled).length;
    ...
    <View style={styles.container}>
      ...
      <View style={[styles.header, { paddingTop: insets.top + 8, borderBottomColor: Colors.borderLight }]}>
        <View style={styles.headerRow}>
          <Pressable onPress={handleBack} hitSlop={10} style={({ pressed }) => [styles.backBtn, pressed && styles.backBtnPressed]} testID="favourites-back">
            <ChevronLeft size={24} color={Colors.text} />
          </Pressable>
          <View style={styles.headerTitleWrap}>
            <View style={styles.headerLabelRow}>
              <Heart size={13} color={accentColor} fill={accentColor} />
              <Text style={[styles.headerLabel, { color: accentColor }]}>Favourite</Text>
            </View>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptySubtitle}>There are no scheduled sessions for this favourite right now. Pull down to refresh the schedule.</Text>
          </View>
        }
        testID="favourite-classes-list"
      />
    </View>
  );
}
